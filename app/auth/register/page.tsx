...
import {
  Eye, EyeOff, Lock, User, Mail, CheckCircle2,
 ...
 * XOR-based comparison avoids early-exit patterns flagged by CWE-208 analysis.
 */
...
function RegisterForm() {
  ...
  const validateToken = useCallback(async () => {
    if (!token) {
      setErrorMsg("No registration token found in the URL. Please use the link from your invitation email.");
      setStage("error");
      return;
    }
    try {
      ...
      if (!res.ok) { setErrorMsg(data.error ?? "Invalid registration link."); setStage("error"); return; }
          ...
          <>
            ...
            <Card className="border border-slate-200/60 shadow-2xl shadow-slate-200/60 rounded-3xl overflow-hidden bg-white/90 backdrop-blur-xl">
              <CardContent className="p-8 text-center space-y-4">
                ...
                <div>
                  <p className="font-black text-slate-900 text-lg">Welcome, {name}!</p>
                  <p className="text-slate-500 text-sm mt-1">Your account is ready. Redirecting to login…</p>
                </div>
                {/* Progress bar */}
            ...
            <Card className="border border-slate-200/60 shadow-2xl shadow-slate-200/60 rounded-3xl overflow-hidden bg-white/90 backdrop-blur-xl">
              ...
              <div className="px-8 pt-6 pb-4 border-b border-slate-100">
                <div className="flex items-center gap-2.5 bg-[#0086D1]/8 border border-[#0086D1]/20 rounded-xl px-4 py-3">
                  <ShieldCheck size={15} className="text-[#0086D1] shrink-0" />
                  <div>
                    <p className="text-[10px] font-black text-[#0086D1] uppercase tracking-widest leading-none mb-0.5">Invited Account</p>
              ...
              <CardContent className="p-8 pt-6">
                <form onSubmit={handleSubmit} className="space-y-5" autoComplete="off">
                  ...
                  <div className="space-y-1.5">
                    <label htmlFor="reg-name" className="text-xs font-bold text-slate-500 uppercase tracking-widest pl-1">
                      Full Name
                    </label>
                  ...
                  {/* Password */}
                  <div className="space-y-1.5">
                    <label htmlFor="reg-password" className="text-xs font-bold text-slate-500 uppercase tracking-widest pl-1">
                      Password
                    </label>
                    <div className="relative">
                  ...
                  {/* Confirm password */}
                  <div className="space-y-1.5">
                    <label htmlFor="reg-confirm" className="text-xs font-bold text-slate-500 uppercase tracking-widest pl-1">
                      Confirm Password
                    </label>
                    <div className="relative">
                      <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={17} />
